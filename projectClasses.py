...
class Gun(pygame.sprite.Sprite):
    ...
    def __init__(self, bullet, listOfImages, screen, delay = 50, filePathForCross = "images/cross.png", filePathForBullet = "images/bullet.png", maxBullet = 12, max_width = 1366, max_height = 766):
        super().__init__()
        # Arguments
    ...
    def gunAnimation(self):
        if Gun.timeDelayGun == 0:
            ...
            if Gun.gunX > ((self.max_width / 2) + 50):
            ...
            elif Gun.gunX < ((self.max_width / 2) - 50):
    ...
    def bulletRemaining(self):
        ...
        if self.bulletInGun <= 12:
            for i in range(int(self.bulletInGun)):
                self.screen.blit(self.bullet, (self.max_width - 90 - 20 * i, 30))
                text = f"BulletInventory: {self.maxBullet}"
                textSurface = font.render(text, False, 'Black')
                self.screen.blit(textSurface, (self.max_width - 10 * len(text) - 60, 75))
        else:
            self.screen.blit(textSurface, (self.max_width - 10 * len(text) - 60, 45))
            text = f"BulletInventory: {self.maxBullet}"
            textSurface = font.render(text, False, 'Black')
            self.screen.blit(textSurface, (self.max_width - 10 * len(text) - 60, 75))
            self.screen.blit(self.bullet, (self.max_width - 90, 30)) 
    ...
    def gunReload(self):
        if self.maxBullet > 0:  # Check if there are bullets in the inventory
            ...
            if self.maxBullet >= needed_bullets:
                self.bulletInGun += needed_bullets
                self.maxBullet -= needed_bullets
            else:
